...
mod tests {
    ...
    fn test_find_files_by_extensions() {
        ...
        assert!(
            txt_files
                .iter()
                .any(|p| p.file_name().unwrap() == "test1.txt")
        );
        assert!(
            txt_files
                .iter()
                .any(|p| p.file_name().unwrap() == "test3.txt")
        );
        ...
        assert!(
            rs_files
                .iter()
                .any(|p| p.file_name().unwrap() == "test2.rs")
        );
        assert!(
            rs_files
                .iter()
                .any(|p| p.file_name().unwrap() == "test5.rs")
        );
        ...
        assert!(
            multi_ext_files
                .iter()
                .any(|p| p.file_name().unwrap() == "test4.md")
        );
    ...
    fn test_read_file_size_limit() {
        ...
        let large_content = "X".repeat(11 * 1024 * 1024); // 11MB
        {
            ...
            file.write_all(large_content.as_bytes())
                .expect("Failed to write to large file");
        }
    ...
    fn test_read_file_with_unicode() {
        ...
        let unicode_content = "Hello, 世界! Привет, мир! 👋";
        {
            ...
            file.write_all(unicode_content.as_bytes())
                .expect("Failed to write unicode to file");
        }
